...
void printCmdSummary() {
    cout<<"Welcome! Please see the instruction below to enter data!"<<endl;
    cout<< "Valid commands are [i](insert new entry), [c](change existed entry), [l](lookup desired entry), "
    << endl;
    cout << "                  [r](remove desired entry) , [p](print out the entire table), [s](size of entries),"
    << endl;
    cout << "                  [t](statistics about this table) , [q](quit program), [h](provide commands)."
    << endl;
}
//check whether the entered key is NULL
bool isValidString(string key){
    if(key!=""){
        return true;
    }
...
//check whether the socre is less than 0
//if not, make client re-enter
bool isValidInt(int key){
    if (key<0)
    {
